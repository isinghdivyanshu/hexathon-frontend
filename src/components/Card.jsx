...
export default function Card({ name, id, price, url, incart }) {
  ...
  const addToCart = async () => {
    try {
      const response = await axios.post("/api/v1/carts/"+id, {}, {
        headers: {
  ...
  return (
    <div className="flex flex-col min-w-[16rem] bg-[#411623] border-2 border-white/10 p-4 rounded-lg">
      ...
      {!added ? (
        <button onClick={addToCart} className="bg-red-600 hover:bg-red-700 px-8 py-1 text-sm rounded-md mt-4 font-DelaGothicOne">
        {price} <img src={hexcoin} className="inline w-[1 5%] h-[90%]" />
      </button>
      ) : (
        <button onClick={removeFromCart} className="bg-red-300 hover:bg-red-700 px-8 py-1 text-sm rounded-md mt-4 font-DelaGothicOne">
        Remove From Cart
      </button>
      )}
    </div>
